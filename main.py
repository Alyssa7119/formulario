...
def login():
        ...
        if request.method == 'POST':
            ...
            for user in users_db:
                if form_login == user.email:
                    if access_fail >= 2:
                        return render_template('login.html', error="Cuenta bloqueada temporalmente. Vuelve a ingresar a la pagina e intentalo de nuevo.")
                    ...
                    if form_password == user.password:
                        ...
                        access_fail += 1
                        attempts = 3 - access_fail
                        return render_template('login.html', error=f"Contraseña incorrecta. Te quedan {attempts} intentos.")

            return render_template('login.html', error="Acceso inválido. Crea una cuenta e intenta de nuevo.")

        access_fail = 0
        return render_template('login.html', error = "Rellena los campos")
